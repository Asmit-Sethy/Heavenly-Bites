...
const Menu = () => {
  ...
  // Only try to find a product if filterby is present
  const productDisplay = filterby
    ? productData.find(el => String(el._id) === String(filterby))
    : null;

  const handleAddCartProduct = () => {
    if (!productDisplay) return; // nothing selected
    dispatch(addCartItem(productDisplay));
  };
  ...
  return (
    <div className="p-2 md:p-4">
      {/* Show product details ONLY when a valid product is selected */}
      {productDisplay && (
        <div className="w-full max-w-4xl m-auto md:flex bg-purple-200 rounded-md shadow drop-shadow">
          <div className="max-w-sm overflow-hidden cursor-pointer w-full p-6">
            <img
          ...
          <div className="flex flex-col gap-2">
            <h3 className="font-semibold mt-5 capitalize text-2xl md:text-4xl">
              {productDisplay.name}
            </h3>
            <p className="text-slate-700 capitalize font-medium text-2xl">
            ...
            <div className="flex gap-3">
              <button
                onClick={handleBuy}
                className="bg-pink-500 hover:bg-green-500 text-white mt-2 font-bold rounded-md py-2 px-5 my-4"
              >
                Buy
              </button>
              <button
      ...
      {/* Always show the product list below */}
      <AllProduct heading={productDisplay ? 'You Might Like:' : 'Menu'} />
